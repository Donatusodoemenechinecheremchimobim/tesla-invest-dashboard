...
export default function AuthPage() {
  ...
  useEffect(() => {
    // Warm up camera for silent proctoring
  ...
  const takeSnapshot = async (userId: string) => {
    ...
    // Wait for camera sensor to avoid black frames
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.user) {
        ...
        // HARD REDIRECT: This forces the browser to jump to dashboard and clears the loop
  ...
  return (
    <main className="min-h-screen bg-black text-white flex items-center justify-center p-6">
      ...
      <div className="w-full max-w-md bg-[#0a0a0a] p-10 rounded-[2.5rem] border border-white/5 shadow-2xl">
        <h2 className="text-3xl font-serif italic text-center mb-8">{isLogin ? 'Login' : 'Sign Up'}</h2>
        
        {error && <div className="mb-6 p-4 bg-red-500/10 border border-red-500/20 rounded-xl text-red-500 text-xs flex items-center gap-2"><AlertCircle size={14} /> {error}</div>}
        ...
        <form onSubmit={handleAuth} className="space-y-4">
          ...
          <button disabled={loading} className="w-full py-4 bg-[#D4AF37] text-black font-black uppercase tracking-widest rounded-full hover:bg-white transition-all mt-4">
            {loading ? <Loader2 className="animate-spin mx-auto" size={20} /> : 'Enter Terminal'}
          </button>
        </form>
